...
function staticHandler(request, response, filename) {
    fs.exists(filename, (exists) => {
        ...
        if(request.headers["If-None-Match"] === etag)
            return sendResponseCode(response, 304);
        
        fs.readFile(filename, "binary", (err, file) => {
            if(err) return sendResponseCode(response, 500, err);

            response.writeHead(200, {"Content-Type": getMimeType(filename, this.Config.mimetypes), "Content-Length": stat.size, "Service-Worker-Allowed": "/"});
...
module.exports.WebServer = class extends EventEmitter {
    ...
    constructor(conf) {
        ...
        this.sendResponseCode = sendResponseCode; // unbound
