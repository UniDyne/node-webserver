...
function sendDirectory(request, response, filename) {
    ...
    response.write(`<script type="text/javascript">document.addEventListener('keyup',(e)=>{if(e.keyCode==37)j--;if(e.keyCode==39)j++;flip();});function flip(){j=(j<0?BROWSE.files.length-1:j>=BROWSE.files.length?0:j);document.getElementById('xx').style['background-image']='url('+BROWSE.files[j]+')';}window.j=0;window.BROWSE=${JSON.stringify(listing)};</script>`);
...
function browseHandler(request, response, filename) {
    console.log(`browseHandler: ${filename}`);
